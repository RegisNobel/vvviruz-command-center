        ...
        <div className="mx-auto flex max-w-[1280px] items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
          <Link className="min-w-0" href="/">
            <div className="inline-flex items-center gap-3">
              <span className="relative block h-11 w-11 overflow-hidden rounded-full border border-[#c9a347]/40 bg-[#c9a347]/10">
                <Image
                  alt={
              ...
              <div className="min-w-0">
                <p className="truncate text-lg font-semibold tracking-[0.04em] text-[#f3eddf]">
                  {siteSettings.artist_name}
                </p>
                <p className="truncate text-xs uppercase tracking-[0.24em] text-[#8d949d]">
          ...
          <nav className="flex flex-wrap items-center gap-2">
            {navItems.map((item) => (
              <Link
                className="rounded-full border border-white/10 bg-white/[0.03] px-4 py-2 text-sm font-semibold text-[#e7dfcf] transition hover:border-[#c9a347]/40 hover:bg-[#c9a347]/10 hover:text-[#f6f0e4]"
                href={item.href}
                key={item.href}
              >
                {item.label}
